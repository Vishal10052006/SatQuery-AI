...
import {
  Activity, ArrowUpRight, Check, ChevronDown, CircleDot, Database, FileImage,
  Globe2, Layers3, Loader2, MapPin, MessageSquare, Network, Radar, Search,
...
const PIPELINE_STEPS = [
  { label: 'Query', icon: Search }, { label: 'Intent', icon: Workflow }, { label: 'Route', icon: Network },
  { label: 'Specialist', icon: Radar }, { label: 'Evidence', icon: Layers3 }, { label: 'Answer', icon: Sparkles },
] as const;
const SPECIALISTS = [
  { id: 'M1', name: 'EarthDial VQA', role: 'Visual understanding' }, { id: 'M2', name: 'Change Detection', role: 'Bi-temporal analysis' },
  { id: 'M3', name: 'Optical + SAR', role: 'Sensor fusion' }, { id: 'M4', name: 'AI Agent', role: 'Intent + routing' },
  { id: 'M5', name: 'GIS / Evidence', role: 'Spatial provenance' }, { id: 'M6', name: 'Interface', role: 'Judge experience' },
] as const;
...
const confidenceLabel = (value: number): string => `${Math.round(Math.max(0, Math.min(1, Number.isFinite(value) ? value : 0)) * 100)}%`;
...
const routeLabel = (result: AnalysisResponse | null, _specialistId: string, demo: boolean): string => {
  if (demo) return 'Demo response';
  const tools = result?.toolResults ?? [];
  return tools.length ? `M4 → ${tools.map((item) => item.tool.replaceAll('_', ' ')).join(' → ')}` : 'M4 response';
};
...
const buildSummaryRows = (result: AnalysisResponse | null, query: string): SummaryRow[] => {
  ...
  const area = Number.isFinite(areaSqM) && areaSqM > 0 ? `${(areaSqM / 10000).toFixed(2)} ha` : 'Not available';
  ...
  return [
    { metric: 'Changed surface', before: '—', after: '—', result: area },
    { metric: 'Changed regions', before: '—', after: '—', result: Number.isFinite(regions) ? String(regions) : 'Not available' },
    { metric: 'Constructed area', before: 'Not classified', after: 'Not classified', result: isConstruction ? area : 'Not separately classified' },
    { metric: 'River / water features', before: 'Not classified', after: 'Not classified', result: waterExcluded ? 'Water excluded from land-change mask' : 'Not classified by M2' },
    { metric: 'Spatial reference', before: georef ? 'Georeferenced' : 'Pixel space', after: georef ? 'Georeferenced' : 'Pixel space', result: georef ? 'Geographic evidence available' : 'No geographic coordinates' },
    { metric: 'Detector', before: 'Baseline', after: 'Baseline', result: String(data.detector ?? m2?.data?.model ?? 'M2 change detector') },
    { metric: 'Confidence', before: '—', after: '—', result: confidenceLabel(result.confidence) },
  ];
};
...
export const App: React.FC = () => {
  ...
  useEffect(() => {
    let active = true;
    const checkHealth = async () => { const result = await checkBackendHealth(); if (active) setBackendOnline(result.isOnline); };
    void checkHealth(); const timer = window.setInterval(() => void checkHealth(), 15000);
  ...
  const summaryRows = useMemo(() => buildSummaryRows(currentResult, query), [currentResult, query]);
  ...
  const externalMapUrl = useMemo(() => {
    ...
    const { lat, lng } = coordinates; const bbox = currentResult?.boundingBox;
    const west = bbox?.west ?? lng - 0.12, south = bbox?.south ?? lat - 0.08, east = bbox?.east ?? lng + 0.12, north = bbox?.north ?? lat + 0.08;
  ...
  const loadScenario = () => {
    const scenario = SAMPLE_SCENARIOS.find((item) => item.id === 'change-detection-flood'); if (!scenario) return;
    setIsDemoMode(true); setQuery(scenario.query); setCurrentResult(scenario.mockResponse); setPipelineStage('completed'); setErrorMessage(null);
    setSlot1({ name: 'Before image', previewUrl: scenario.images.before ?? '' }); setSlot2({ name: 'After image', previewUrl: scenario.images.after ?? '' });
  };
  const handleUpload = (slot: 1 | 2, file?: File) => { if (!file) return; const next = { file, name: file.name, previewUrl: toFilePreview(file) }; if (slot === 1) setSlot1(next); else setSlot2(next); setCurrentResult(null); setPipelineStage('idle'); setErrorMessage(null); };
  const handleAnalyze = async () => {
    setErrorMessage(null); if (!canAnalyze) { setErrorMessage(needsPair ? 'Add both source images before analysis.' : 'Add a satellite image before analysis.'); return; }
    try {
      setCurrentResult(null); setPipelineStage('routing'); await new Promise((resolve) => window.setTimeout(resolve, 180)); setPipelineStage('analyzing');
      ...
      setPipelineStage('evidence'); setCurrentResult(result); setHistory((items) => [result, ...items].slice(0, 8)); setPipelineStage(result.status === 'error' ? 'error' : 'completed');
    } catch (error) { setPipelineStage('error'); setErrorMessage(error instanceof Error ? error.message : 'Analysis pipeline failed.'); }
  };
  const reset = () => { setQuery(EXAMPLE_QUERIES[0]); setSlot1({ name: '' }); setSlot2({ name: '' }); setCurrentResult(null); setPipelineStage('idle'); setErrorMessage(null); };
  const sourceImages = needsPair ? [{ label: specialist.id === 'M3' ? 'OPTICAL' : 'BEFORE', src: slot1.previewUrl }, { label: specialist.id === 'M3' ? 'SAR' : 'AFTER', src: slot2.previewUrl }] : [{ label: 'SATELLITE IMAGE', src: slot1.previewUrl }];
  const trace = currentResult?.reasoningSteps?.length ? currentResult.reasoningSteps : actualTool ? [`${actualTool.tool.replaceAll('_', ' ')} → ${actualTool.status}`] : [];

  return <div className="app-shell">
    <header className="topbar"><div className="brand"><div className="brand-icon"><Radar size={18} /></div><div><strong>SATQUERY <span>AI</span></strong><small>Natural-language satellite intelligence</small></div></div><div className="topbar-actions"><span className={`system-status ${backendOnline ? 'online' : 'offline'}`}><CircleDot size={11} /> {backendOnline ? 'LIVE SYSTEM' : 'BACKEND OFFLINE'}</span><button className={`mode-button ${isDemoMode ? 'active' : ''}`} type="button" onClick={() => setIsDemoMode((value) => !value)}>{isDemoMode ? 'Demo data' : 'Live backend'} <ChevronDown size={13} /></button><button className="icon-button" type="button" onClick={reset} aria-label="Reset"><X size={16} /></button></div></header>
    <main className="workspace">
      <section className="intro"><div><p className="kicker"><Sparkles size={13} /> JUDGE WORKSPACE</p><h1>Ask. Analyze. <em>See the evidence.</em></h1><p className="intro-copy">Ask questions about satellite imagery in natural language and receive AI-driven analysis with visual and geospatial evidence.</p></div><div className="intro-proof"><span><ShieldCheck size={14} /> Real backend artifacts</span><span><Network size={14} /> M4 routing</span><span><Globe2 size={14} /> GIS-ready</span></div></section>
      <section className="command-grid"><div className="panel data-panel"><div className="panel-head"><div><p className="overline">01 · INPUT</p><h2>Satellite data</h2></div><button className="text-button" type="button" onClick={loadScenario}>Load demo pair <ArrowUpRight size={14} /></button></div><div className={`source-grid ${needsPair ? 'pair' : 'single'}`}><UploadTile label={needsPair ? (specialist.id === 'M3' ? 'Optical image' : 'Before image') : 'Satellite image'} slot={slot1} onFile={(file) => handleUpload(1, file)} />{needsPair && <UploadTile label={specialist.id === 'M3' ? 'SAR image' : 'After image'} slot={slot2} onFile={(file) => handleUpload(2, file)} />}</div></div>
      <div className="panel query-panel"><div className="panel-head"><div><p className="overline">02 · QUERY</p><h2>Ask SatQuery</h2></div><span className="route-hint"><span /> M4 → {specialist.id}</span></div><div className="query-box"><MessageSquare size={16} /><textarea value={query} onChange={(event) => setQuery(event.target.value)} aria-label="Satellite intelligence query" placeholder="What changed between these images?" /></div><div className="suggestions">{EXAMPLE_QUERIES.slice(0, 3).map((example) => <button key={example} type="button" onClick={() => setQuery(example)}>{example}</button>)}</div><div className="query-footer"><span className="input-note">{needsPair ? 'Two observations required' : 'One observation required'} · JPG / PNG / GeoTIFF</span><button className="primary-button" type="button" onClick={handleAnalyze} disabled={isRunning || !canAnalyze}>{isRunning ? <Loader2 className="spin" size={16} /> : <Sparkles size={16} />}{isRunning ? 'Analyzing' : 'Analyze'}</button></div>{errorMessage && <div className="error-line">{errorMessage}</div>}</div></section>
      <section className="pipeline-strip"><div className="pipeline-title"><span className="overline">03 · AGENT</span><strong>Execution</strong></div><div className="pipeline-steps">{PIPELINE_STEPS.map((step, index) => { const done = pipelineStage === 'completed' || stageRank[pipelineStage] > index; const active = pipelineStage !== 'completed' && ((pipelineStage === 'routing' && index < 3) || (pipelineStage === 'analyzing' && index === 3) || (pipelineStage === 'evidence' && index === 4)); const Icon = step.icon; return <div className={`pipeline-step ${done ? 'done' : ''} ${active ? 'active' : ''}`} key={step.label}><span>{done ? <Check size={13} /> : <Icon size={13} />}</span><small>{step.label}</small>{index < PIPELINE_STEPS.length - 1 && <i />}</div>; })}</div><span className={`pipeline-state ${pipelineStage}`}>{stageLabel(pipelineStage)}</span></section>
      {currentResult && <>
        ...
        {summaryRows.length > 0 && <section className="analysis-summary-section"><div className="section-heading"><div><p className="overline">04A · SUMMARY</p><h3>Change analysis overview</h3></div><span>Derived from M2 evidence</span></div><div className="analysis-summary-table-wrap"><table className="analysis-summary-table"><thead><tr><th>Metric</th><th>Before</th><th>After</th><th>Analysis</th></tr></thead><tbody>{summaryRows.map((row) => <tr key={row.metric}><td>{row.metric}</td><td>{row.before}</td><td>{row.after}</td><td>{row.result}</td></tr>)}</tbody></table></div><p className="analysis-summary-note">The table intentionally omits raw pixel counts. “River / water features” are shown as unclassified unless a dedicated hydrology/object detector returns a defensible count.</p></section>}
