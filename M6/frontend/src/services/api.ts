...
/** Collect image-like artifacts from the stable M4 response contract. */
...
export async function executeSatelliteAnalysis(
  ...
  try {
    ...
    if (error instanceof Error && error.message.includes('Failed to fetch')) {
      throw new Error(`Unable to reach M5 FastAPI Backend at ${API_BASE_URL}.`);
    }
    throw error;
  }
}
...
function normalizeBackendResponse(raw: any, payload: AnalysisPayload): AnalysisResponse {
  ...
  return {
    ...
    confidence: typeof raw.confidence === 'number' ? (raw.confidence > 1 ? raw.confidence / 100 : raw.confidence) : 0,
    ...
    error: raw.error,
    coordinates: raw.coordinates || (raw.lat != null && raw.lng != null ? { lat: raw.lat, lng: raw.lng, locationName: raw.location_name } : undefined),
    ...
    sensorMetadata: raw.sensor_metadata || {
      ...
      crs: raw.crs || 'Not returned by backend',
    },
  };
}
